...
def test_rank_series_by_source(mock_requests_get):
    ...
    for idx, series in enumerate(
        lib.rank_series_by_source(
            MOCK_TOKEN, MOCK_HOST, [full_data_series, partial_selection]
        )
    ):
...
def test_get_ancestor(mock_requests_get, lookup_mocked):
    ...
    assert lib.get_ancestor(
        MOCK_TOKEN, MOCK_HOST, "metrics", 1, include_details=True
    ) == [
        {
            "id": 3,
            "name": "parent",
    ...
    assert lib.get_ancestor(
        MOCK_TOKEN, MOCK_HOST, "items", 1, include_details=False
    ) == [{"id": 3}, {"id": 4}]
...
def test_get_descendant_for_items(mock_requests_get, lookup_mocked):
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, ITEMS_ENTIRY, 4, include_details=False
    ) == [
        {"id": 1},
        {"id": 2},
        {"id": 3},
...
def test_get_descendant_for_metrics(mock_requests_get, lookup_mocked):
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, METRICS_ENTITY, 4, include_details=True
    ) == [
        {
            "id": 1,
            "name": "metric 1",
...
def test_get_descendant_for_regions(mock_requests_get, lookup_mocked):
    ...
    mock_requests_get.assert_called_with(
        ...
        params={
            "ids": [3],
            "distance": DESCENDANT_DISTANCE_NAN,
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_details=True
    ) == [
        {
            "id": 1,
            "name": "region 1",
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_details=False
    ) == [{"id": 1}, {"id": 2}]
    mock_requests_get.assert_called_with(
        f"https://{MOCK_HOST}/v2/{REGIONS_ENTITY}/contains",
        params={
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_historical=True
    ) == [
        {
            "id": 1,
            "name": "region 1",
...
def test_get_geo_json(geojsons_mocked):
    ...
    expected_return = {
        ...
        "geometries": [
            {
                "type": "MultiPolygon",
                "coordinates": [[[[-155.651382446, 20.1647224430001]]]],
            }
        ],
    }
...
def test_reverse_geocode_points(mock_requests_post):
    ...
    initialize_requests_mocker_and_get_mock_data(
        mock_requests_post, {"data": expected_output}
    )

    result = lib.reverse_geocode_points(
