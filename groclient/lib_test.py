...
def initialize_requests_mocker_and_get_mock_data(
    mock_requests_get, mock_data={'data': [{'name': 'obj1'}, {'name': 'obj2'}, {'name': 'obj3'}]}
):
...
def test_single_lookup(mock_requests_get):
    api_response = {'data': {'12345': {'id': 12345, 'name': 'Vegetables', 'contains': [67890], 'belongsTo': []}}}
...
def test_lookup_with_numpy(mock_requests_get):
    ...
    assert lib.lookup(MOCK_TOKEN, MOCK_HOST, 'items', np.array([12345])[0]) == expected_return['12345']
...
def test_get_data_points(mock_requests_get):
    list_of_series_format_data = [
        {'series': {}, 'data': [['2000-01-01', '2000-12-31', 1, None, 15, None, '2001-12-31']]}
    ]
    ...
    initialize_requests_mocker_and_get_mock_data(mock_requests_get, mock_data=list_of_series_format_data)
...
def test_list_of_series_to_single_series():
    ...
    assert (
        'belongs_to'
        in lib.list_of_series_to_single_series(
            [{'series': {'unitId': 4, 'belongsTo': {'itemId': 22}}, 'data': [['2001-01-01', '2001-12-31', 123]]}],
            add_belongs_to=True,
        )[0]
    )
    ...
    assert (
        'belongs_to'
        not in lib.list_of_series_to_single_series(
            [{'series': {'unitId': 4, 'belongsTo': {'itemId': 22}}, 'data': [['2001-01-01', '2001-12-31', 123]]}],
            add_belongs_to=False,
        )[0]
    )
    ...
    assert (
        len(
            lib.list_of_series_to_single_series(
                [
                    {
                        'series': {
                            'unitId': 4,
...
def test_search_and_lookup(search_mocked, lookup_mocked):
    ...
    search_and_lookup_result = list(lib.search_and_lookup(MOCK_TOKEN, MOCK_HOST, 'regions', 'test123'))

    assert search_and_lookup_result == [LOOKUP_MAP['regions']['1'], LOOKUP_MAP['regions']['2']]
...
def test_rank_series_by_source(mock_requests_get):
    ...
    partial_selection = {'metric_id': 2540047, 'item_id': 1457, 'region_id': [13474, 13475]}
    ...
    for idx, series in enumerate(
        lib.rank_series_by_source(MOCK_TOKEN, MOCK_HOST, [full_data_series, partial_selection])
    ):
...
def lookup_mock(MOCK_TOKEN, MOCK_HOST, entity_type, entity_ids):
    ...
    if isinstance(entity_ids, list):
        return {str(entity_id): LOOKUP_MAP[entity_type][str(entity_id)] for entity_id in entity_ids}
...
def test_get_ancestor(mock_requests_get, lookup_mocked):
    ...
    assert lib.get_ancestor(MOCK_TOKEN, MOCK_HOST, 'items', 1, include_details=False) == [{'id': 3}, {'id': 4}]
...
@mock.patch('groclient.lib.lookup')
@mock.patch('requests.get')
def test_get_descendant_for_items(mock_requests_get, lookup_mocked):
    mock_requests_get.return_value.json.return_value = {'data': {'4': [1, 2, 3]}}
    mock_requests_get.return_value.status_code = 200
    lookup_mocked.side_effect = lookup_mock
    ITEMS_ENTIRY = 'items'

    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, ITEMS_ENTIRY, 4) == [
        {'id': 1, 'name': 'item 1', 'contains': [], 'belongsTo': [3], 'definition': 'def1'},
        {'id': 2, 'name': 'item 2', 'contains': [], 'belongsTo': [3], 'definition': 'def2'},
        {'id': 3, 'name': 'parent', 'contains': [1, 2], 'belongsTo': [4], 'definition': 'def3'},
    ]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{ITEMS_ENTIRY}/contains',
        params={'ids': [4], 'distance': DESCENDANT_DISTANCE_NAN},
        headers=mock.ANY,
        timeout=mock.ANY,
        stream=mock.ANY,
    )

    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, ITEMS_ENTIRY, 4, include_details=False) == [
        {'id': 1},
        {'id': 2},
        {'id': 3},
    ]
    ...
    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, ITEMS_ENTIRY, 4, distance=1) == [
        {'id': 3, 'name': 'parent', 'contains': [1, 2], 'belongsTo': [4], 'definition': 'def3'}
    ]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{ITEMS_ENTIRY}/contains',
        params={'ids': [4], 'distance': 1},
        headers=mock.ANY,
        timeout=mock.ANY,
        stream=mock.ANY,
    )
...
@mock.patch('groclient.lib.lookup')
@mock.patch('requests.get')
def test_get_descendant_for_metrics(mock_requests_get, lookup_mocked):
    mock_requests_get.return_value.json.return_value = {'data': {'4': [1, 2, 3]}}
    mock_requests_get.return_value.status_code = 200
    lookup_mocked.side_effect = lookup_mock
    METRICS_ENTITY = 'metrics'

    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, METRICS_ENTITY, 4, include_details=True) == [
        {'id': 1, 'name': 'metric 1', 'contains': [], 'belongsTo': [3], 'definition': 'def1'},
        {'id': 2, 'name': 'metric 2', 'contains': [], 'belongsTo': [3], 'definition': 'def2'},
        {'id': 3, 'name': 'parent', 'contains': [1, 2], 'belongsTo': [4], 'definition': 'def3'},
    ]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{METRICS_ENTITY}/contains',
        params={'ids': [4], 'distance': DESCENDANT_DISTANCE_NAN},
        headers=mock.ANY,
        timeout=mock.ANY,
        stream=mock.ANY,
...
@mock.patch('groclient.lib.lookup')
@mock.patch('requests.get')
def test_get_descendant_for_regions(mock_requests_get, lookup_mocked):
    mock_requests_get.return_value.json.return_value = {'data': {'4': [1, 2, 3]}}
    mock_requests_get.return_value.status_code = 200
    lookup_mocked.side_effect = lookup_mock
    REGIONS_ENTITY = 'regions'

    mock_requests_get.return_value.json.return_value = {'data': {'3': [1, 2]}}
    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3) == [
        {'id': 1, 'name': 'region 1', 'contains': [], 'belongsTo': [3], 'historical': True},
        {'id': 2, 'name': 'region 2', 'contains': [], 'belongsTo': [3], 'historical': False},
    ]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{REGIONS_ENTITY}/contains',
        params={'ids': [3], 'distance': DESCENDANT_DISTANCE_NAN, 'includeHistorical': True},
        headers=mock.ANY,
        timeout=mock.ANY,
        stream=mock.ANY,
    )

    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_details=True) == [
        {'id': 1, 'name': 'region 1', 'contains': [], 'belongsTo': [3], 'historical': True},
        {'id': 2, 'name': 'region 2', 'contains': [], 'belongsTo': [3], 'historical': False},
    ]
    mock_requests_get.assert_called_with(
    ...
    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_details=False) == [{'id': 1}, {'id': 2}]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{REGIONS_ENTITY}/contains',
        params={'ids': [3], 'distance': DESCENDANT_DISTANCE_NAN, 'includeHistorical': True},
        headers=mock.ANY,
    ...
    assert lib.get_descendant(MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_historical=True) == [
        {'id': 1, 'name': 'region 1', 'contains': [], 'belongsTo': [3], 'historical': True},
        {'id': 2, 'name': 'region 2', 'contains': [], 'belongsTo': [3], 'historical': False},
    ]
    mock_requests_get.assert_called_with(
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_historical=True, include_details=True
    ) == [
        {'id': 1, 'name': 'region 1', 'contains': [], 'belongsTo': [3], 'historical': True},
        {'id': 2, 'name': 'region 2', 'contains': [], 'belongsTo': [3], 'historical': False},
    ]
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_historical=False, include_details=False
    ) == [{'id': 2}]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{REGIONS_ENTITY}/contains',
        params={'ids': [3], 'distance': DESCENDANT_DISTANCE_NAN, 'includeHistorical': False},
        headers=mock.ANY,
        timeout=mock.ANY,
        stream=mock.ANY,
    )
    ...
    assert lib.get_descendant(
        MOCK_TOKEN, MOCK_HOST, REGIONS_ENTITY, 3, include_historical=False, include_details=True
    ) == [{'id': 2, 'name': 'region 2', 'contains': [], 'belongsTo': [3], 'historical': False}]
    mock_requests_get.assert_called_with(
        f'https://{MOCK_HOST}/v2/{REGIONS_ENTITY}/contains',
        params={'ids': [3], 'distance': DESCENDANT_DISTANCE_NAN, 'includeHistorical': False},
...
def test_get_geo_centre(mock_requests_get):
    US_data = {"regionId": 1215, "regionName": "United States", "centre": [39.8333, -98.5855]}
    api_response = {'data': [US_data]}
    initialize_requests_mocker_and_get_mock_data(mock_requests_get, api_response)
        ...
        'data': [
            {
                ...
                "geojson": {"type": "MultiPolygon", "coordinates": [[[[-88.201896667, 35.0088806150001]]]]},
            },
            {
                "regionId": 13052,
                "regionName": "Alaska",
                "centre": [64.2386, -152.279],
                "geojson": {"type": "MultiPolygon", "coordinates": [[[[-179.07043457, 51.2564086920001]]]]},
            },
        ]
    }
    expected_return = [
...
def test_get_area_weighting_series_names(mock_requests_get):
    ...
    initialize_requests_mocker_and_get_mock_data(mock_requests_get, api_response)
    assert lib.get_area_weighting_series_names(MOCK_TOKEN, MOCK_HOST) == [
        "CPC_max_temp_daily",
        "CPC_min_temp_daily",
        "ET_PET_monthly",
    ]
...
def test_get_area_weighting_weight_names(mock_requests_get):
    ...
    initialize_requests_mocker_and_get_mock_data(mock_requests_get, api_response)
    assert lib.get_area_weighting_weight_names(MOCK_TOKEN, MOCK_HOST) == [
        "Almonds (CA only)",
        "Bananas (ha)",
        "Canola (ha)",
    ]
