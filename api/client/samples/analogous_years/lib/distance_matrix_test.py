...
def create_test_dates():
    return [
        {"initial_date": "2018-04-01", "final_date": "2018-08-01"},
...
def test_scaled_labeled_method_distances_invalid_date():
    ...
    with pytest.raises(KeyError):
        distance_matrix.scaled_labeled_method_distances(
            create_test_data(), invalid_initial_date, invalid_final_date, method
        )
...
def test_scaled_labeled_method_distances_invalid_method():
    ...
    with pytest.raises(ValueError):
        distance_matrix.scaled_labeled_method_distances(
            create_test_data(), initial_date, final_date, invalid_method
        )
